...
@Component({
  ...
  template: `<ul class="pagination customPagination">
    <li
      class="page-item disabled ng-star-inserted"
      [ngClass]="{ disabled: viewModel.PageNo == 1 }">
      <a
        aria-label="Previous"
        (click)="selectPageNumber(viewModel.PageNo - 1)"
        class="page-link"
        tabindex="-1"
        aria-disabled="true">
    ...
    <li
      *ngFor="let pgnum of viewModel.totalPages"
      class="page-item ng-star-inserted"
      [ngClass]="{ active: viewModel.PageNo == pgnum }">
      <a (click)="selectPageNumber(pgnum)" class="page-link ng-star-inserted">{{
        pgnum
      }}</a>
    </li>
    <li
      ...
      [ngClass]="{
        disabled:
          viewModel.PageNo ===
          viewModel.totalPages[viewModel.totalPages.length - 1]
      }">
      <a
        (click)="selectPageNumber(viewModel.PageNo + 1)"
        aria-label="Next"
...
export class PaginationComponent
  extends BaseComponent<PaginationViewModel>
  implements OnInit
{
