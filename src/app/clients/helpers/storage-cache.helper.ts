...
export class StorageCache {
  ...
  public GetResponseFromDbIfPresent = async <OutResp>(
    ...
    if (
      ...
      let cacheItem = await this.GetCacheItemIfPresent<ApiResponse<OutResp>>(
        fullUrlToHit
      );
      if (cacheItem != null) {
        return cacheItem;
  ...
  public AddOrUpdateResponseInCacheIfApplicable = async <OutResp>(
    ...
    if (
      ...
      this.AddOrUpdateCacheItem<ApiResponse<OutResp>>(
        fullUrlToHit,
        responseEntity
      );
    }
  ...
  protected GetCacheItemIfPresent = async <T>(
    key: string
  ): Promise<T | null> => {
        ...
        if (cacheItem.ValidUptoDateTimeTicks >= new Date().valueOf()) {
          ...
          if (environment.LoggingInfo.cacheLogs)
            CommonLogger.LogTextOrObject(
              `StorageCache - Response Returned form cache -- '${key}'`
            );
          return item;
        } else {
          if (environment.LoggingInfo.cacheLogs)
  ...
  protected AddOrUpdateCacheItem = async <T>(
    ...
    if (key != null && data != null) {
      ...
      if (environment.LoggingInfo.cacheLogs)
        CommonLogger.LogTextOrObject(
          `StorageCache - New Key Added -- '${key}'`
        );
  ...
  private GetMapFromDb = async (): Promise<Map<string, CacheItem>> => {
    let cacheMap: Map<string, CacheItem> | null = null;
    let cacheMapTxt: string | null = await this.storageService.getFromStorage(
  ...
  private SaveOrUpdateMapInDb = async (
